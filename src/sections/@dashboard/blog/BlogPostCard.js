...
BlogPostCard.propTypes = {
  ...
  linkPath: PropTypes.oneOf(['dashboard', 'www']),
};

export default function BlogPostCard({ post, index, linkPath }) {
  ...
  if (isDesktop && latestPost) {
    return (
      <Card>
        ...
        />
        <PostContent title={title} view={view} comment={comment} share={share} createdAt={createdAt} index={index} linkPath={linkPath} />
        <OverlayStyle />
...
export function PostContent({ title, view, comment, share, createdAt, index, linkPath = 'dashboard' }) {
  const isDesktop = useResponsive('up', 'md');

  const linkTo = linkPath === 'dashboard' ? PATH_DASHBOARD.blog.view(paramCase(title)) : `/post/${(paramCase(title))}`;
